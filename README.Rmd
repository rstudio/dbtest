...
test_output <- dbtest::test_databases("conn.yml", pkg_test("character-basic.yml"))
plot_tests(test_output)
```

# Writing Test Files

Writing test files in YAML can be a bit strange, because what `dbtest` expects is text.  For instance, do not use the `!expr` trick that the `config` package uses above for a connection object.  This text will get picked up into the testing process, which will do the following:
